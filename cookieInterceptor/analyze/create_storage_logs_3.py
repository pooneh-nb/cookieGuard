...
def run_exfiltration_process(siteName, storage_dict):
    requests_dir = Path(f"analyze/data/sorted_requests/{siteName}/requestlogs.json")
...
def get_cookie_patams(raw_cookie):
...
def write_storage(transaction, storage_dict, siteName, common_headers):
    ...
    cookie_params = get_cookie_patams(raw_cookie)
    ...
    # cookie name is new. The request is to set a new cookie
        ...
        storage_dict[siteName][cookie_params['name']]['owner'] = {'initiatorURL': initiatorURL, 'accessType': accessType, 'timeStamp': timestamp}
...
def main():
    ...
    # common headers
    common_headers_dir = Path(Path.cwd(), "analyze/data/common_headers.txt")
